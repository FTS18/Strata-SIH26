...
export function CrowdDensityView() {
  ...
  const columns: Column<StopCrowd>[] = [
    { key: 'stopName', header: 'Bus Stop / Terminal' },
    {
      key: 'corridor',
      header: 'Corridor',
      render: (s) => <span className="font-mono text-xs text-[var(--text-secondary)]">{s.corridor}</span>,
    },
    {
      ...
      align: 'right',
      render: (s) => (
        <span className={`font-mono font-bold ${s.waitingCommuters > 50 ? 'text-rose-400' : 'text-[var(--text-primary)]'}`}>
    ...
    },
    {
      ...
      render: (s) => <span className="text-xs text-[var(--text-secondary)]">{s.suggestedDispatch}</span>,
    },
  ];
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[var(--text-secondary)]">· {activeStop.corridor}</span>
                  <span
                ...
                <p className="text-xs text-[var(--text-secondary)]">
                  Advisory: <span className="text-[var(--text-primary)] font-medium">{activeStop.suggestedDispatch}</span> · Available Standby: <span className="text-emerald-400 font-mono font-bold">{activeStop.standbyTripperId}</span>
                </p>
                <p className="text-[11px] font-mono text-[var(--text-muted)]">
                  Nearest Fleet Depot: <span className="text-[var(--text-secondary)]">{activeStop.nearestDepot}</span> · Automatic Frequency Adjustment Active
                </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-[var(--surface-border)] bg-[var(--surface-canvas)] p-5 shadow-2xl font-mono text-xs">
            <div className="flex items-center justify-between border-b border-[var(--surface-border)] pb-3 mb-3">
              ...
              <button onClick={() => setIsAdvisoryModalOpen(false)} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-[var(--surface-border)]">
                ...
                <Button variant="primary" size="sm" onClick={handleBroadcast} className="bg-[#2563eb] text-[var(--text-primary)] hover:bg-[var(--surface-subtle)]">
