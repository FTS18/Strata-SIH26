...
export function WardComplianceView() {
  ...
  const columns: Column<WardCompliance>[] = [
    { key: 'wardName', header: 'Municipal Ward / Division' },
    {
      key: 'zone',
      header: 'Zone',
      render: (w) => <span className="font-mono text-xs text-[var(--text-secondary)]">{w.zone}</span>,
    },
    {
      key: 'pciScore',
    ...
    },
    {
      ...
      header: 'Open Defects',
      align: 'right',
      render: (w) => <span className="font-mono font-bold text-[var(--text-primary)]">{w.openTickets}</span>,
    },
    {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-3 sm:gap-4 overflow-y-auto p-3 sm:p-5 bg-[var(--surface-canvas)] text-[var(--text-primary)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-3.5 shrink-0">
        <div className="md:col-span-2 lg:col-span-2 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-panel)] p-4 flex flex-col justify-between shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-[var(--text-secondary)] uppercase font-mono tracking-wide">
              Citywide Pavement Condition Index (PCI)
            </span>
            <span className="rounded bg-[var(--surface-subtle)] border border-[var(--color-accent-primary)] px-2 py-0.5 text-[10px] font-mono text-[var(--text-secondary)]">
              +4.1% MoM Trend
            </span>
          </div>

          <div className="my-2.5 flex items-baseline gap-3">
            <span className="font-display text-4xl sm:text-5xl font-bold text-[var(--text-primary)] tracking-tight">
              74.2
            </span>
            <span className="text-sm font-mono text-[var(--text-secondary)]">/ 100 (Nominal Condition)</span>
          </div>
        ...
        <MetricCard
          ...
          changeType="positive"
          icon={<CheckCircle className="h-4 w-4 text-[var(--text-secondary)]" />}
        />
      </div>
          ...
          <div className="w-full rounded-xl border border-[var(--surface-border)] bg-[var(--surface-panel)] p-4 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[var(--text-secondary)]">· {activeWard.zone}</span>
                ...
                <p className="text-xs text-[var(--text-secondary)]">
                  Contractor: <span className="text-[var(--text-primary)] font-medium">{activeWard.contractorName}</span> · Open Defects: <span className="text-[var(--text-primary)] font-bold">{activeWard.openTickets}</span> ({activeWard.slaOverdueTickets} SLA Overdue) · Resolved (30d): <span className="text-emerald-400 font-mono font-bold">{activeWard.resolvedThisMonth}</span>
                </p>
                <p className="text-[11px] font-mono text-[var(--text-muted)]">
                  Councillor: <span className="text-[var(--text-secondary)]">{activeWard.wardCouncillor}</span> · SLA Performance Rate: {activeWard.complianceRate}%
                </p>
              </div>
            </div>
            ...
            <div className="flex w-full lg:w-auto items-center gap-2 pt-2 lg:pt-0">
              <Button
                ...
                className="flex-1 lg:flex-initial bg-[#94a3b8] text-[#080e1a] font-bold hover:bg-[#f8fafc] text-xs"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-[var(--surface-border)] bg-[var(--surface-canvas)] p-5 shadow-2xl font-mono text-xs">
            <div className="flex items-center justify-between border-b border-[var(--surface-border)] pb-3 mb-3">
              ...
              <button onClick={() => setIsAuditModalOpen(false)} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div className="rounded border border-[var(--surface-border)] bg-[var(--surface-panel)] p-2.5 space-y-1">
                <p className="text-[var(--text-secondary)]">Councillor: <span className="text-[var(--text-primary)] font-bold">{activeWard.wardCouncillor}</span></p>
                <p className="text-[var(--text-secondary)]">Contractor: <span className="text-[var(--text-primary)]">{activeWard.contractorName}</span></p>
                <p className="text-[var(--text-secondary)]">Open Defects: <span className="text-amber-400 font-bold">{activeWard.openTickets}</span> ({activeWard.slaOverdueTickets} Overdue)</p>
              </div>

              <div>
                <label className="block text-[11px] text-[var(--text-secondary)] mb-1">Audit Determination *</label>
              ...
              <div>
                <label className="block text-[11px] text-[var(--text-secondary)] mb-1">Executive Notes & Directions</label>
                <textarea
                  rows={2}
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-[var(--surface-border)]">
                ...
                <Button variant="primary" size="sm" onClick={handleConfirmAudit} className="bg-[#2563eb] text-[var(--text-primary)] hover:bg-[var(--surface-subtle)]">
