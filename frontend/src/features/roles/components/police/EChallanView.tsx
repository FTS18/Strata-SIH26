    ...
    <div className="flex h-full w-full flex-col gap-3 sm:gap-4 overflow-y-auto p-3 sm:p-5 bg-[#092328] text-[#f0fdf4]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#12544F] pb-3 shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#12544F] border border-[#2A835F] text-[#8BBB92] shrink-0">
      ...
      <div className="grid flex-1 grid-cols-1 lg:grid-cols-12 gap-4 overflow-y-auto">
        ...
        <div className="col-span-1 lg:col-span-5 flex flex-col rounded-xl border border-[#12544F] bg-[#0d3137] overflow-hidden">
          <div className="border-b border-[#12544F] bg-[#092328] px-3.5 py-2 flex justify-between items-center text-xs font-mono">
            <span className="text-[#8BBB92] font-semibold">REGISTERED CITATIONS</span>
            <span className="text-emerald-400 font-bold">{filteredChallans.length} Records</span>
          </div>

          <div className="flex-1 overflow-y-auto divide-y divide-[#12544F]/50">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3 transition-colors cursor-pointer flex flex-col gap-1 ${
                    isSelected ? 'bg-[#12544F]/60 border-l-4 border-l-emerald-400' : 'hover:bg-[#06191c]/50'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs font-bold text-[#f0fdf4]">{c.plate}</span>
                    <span
                  ...
                  <p className="text-[11px] text-[#8BBB92] line-clamp-1">{c.violationType}</p>
                  <div className="flex items-center justify-between text-[10px] font-mono text-[#5b9076]">
                    <span>{c.id}</span>
                    <span className="text-[#f0fdf4] font-bold">{formatCurrency(c.fineAmount)}</span>
                  </div>
                </button>
              );
            })}
        ...
        {activeChallan && (
          <div className="col-span-1 lg:col-span-7 flex flex-col gap-3 rounded-xl border border-[#12544F] bg-[#0d3137] p-4 sm:p-5 overflow-y-auto">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#12544F] pb-3">
              <div>
                <h3 className="text-sm font-semibold text-[#f0fdf4]">
                  Citation Notice: {activeChallan.id}
                </h3>
                <p className="text-xs text-[#8BBB92] font-mono">{activeChallan.violationType}</p>
              </div>
            ...
            <div className="rounded-lg border border-[#12544F] bg-[#06191c] p-3 font-mono text-xs">
              <div className="flex justify-between items-center text-[10px] text-[#8BBB92] border-b border-[#144943] pb-1 mb-2">
              ...
              <div className="flex flex-col items-center justify-center py-3">
                <div className="rounded border-2 border-rose-500/80 bg-[#092328] px-5 py-2.5 text-center shadow-lg">
                  <span className="text-xl font-bold text-[#f0fdf4] tracking-widest">
                    {activeChallan.plate}
                  </span>
                  <p className="text-[9px] text-[#8BBB92] mt-0.5">HSRP SECURITY THREAD CONFIRMED</p>
                </div>
              </div>
              <div className="flex justify-between text-[10px] text-[#5b9076] border-t border-[#144943] pt-1">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs font-mono">
              <div className="rounded-lg border border-[#12544F] bg-[#092328] p-3 space-y-1">
                <span className="text-[#8BBB92]">Statutory Penalty Amount:</span>
                <p className="text-base font-bold text-rose-400">
                  {formatCurrency(activeChallan.fineAmount)}
                </p>
                <p className="text-[10px] text-[#5b9076]">MoRTH Scheduled Fine</p>
              </div>

              <div className="rounded-lg border border-[#12544F] bg-[#092328] p-3 space-y-1">
                <span className="text-[#8BBB92]">Vehicle Specs:</span>
                <p className="text-xs font-bold text-[#f0fdf4]">{activeChallan.vehicleType}</p>
                <p className="text-[10px] text-[#5b9076]">Registered Owner: {activeChallan.ownerName || 'State Transport Authority'}</p>
              </div>
            </div>
            ...
            {/* Action Buttons Bar */}
            <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-[#12544F] mt-auto">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-xl border border-[#12544F] bg-[#092328] p-5 shadow-2xl font-mono text-xs">
            <div className="flex items-center justify-between border-b border-[#12544F] pb-3 mb-4">
              <div className="flex items-center gap-2">
                <FileText className="h-4 w-4 text-[#8BBB92]" />
                <span className="font-bold text-[#f0fdf4] text-sm">Issue Manual Digital E-Challan</span>
              </div>
              <button
            ...
            <form onSubmit={handleCreateChallan} className="space-y-3">
              <div>
                <label className="block text-[11px] text-[#8BBB92] mb-1">License Plate Number *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] text-[#8BBB92] mb-1">Vehicle Classification</label>
                  <input
                    type="text"
                    value={newType}
                ...
                <div>
                  <label className="block text-[11px] text-[#8BBB92] mb-1">Fine Amount (INR) *</label>
                  <input
                    type="number"
                    required
                    value={newFine}
              ...
              <div>
                <label className="block text-[11px] text-[#8BBB92] mb-1">Specific Motor Vehicle Infraction *</label>
                <select
                  value={newViolation}
                  onChange={(e) => setNewViolation(e.target.value)}
                  className="w-full rounded border border-[#12544F] bg-[#0d3137] px-3 py-2 text-[#f0fdf4] outline-none text-xs"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] text-[#8BBB92] mb-1">Observed Speed (km/h)</label>
                  <input
                    type="number"
                    value={newSpeed}
                ...
                <div>
                  <label className="block text-[11px] text-[#8BBB92] mb-1">Corridor Speed Limit (km/h)</label>
                  <input
                    type="number"
                    value={newLimit}
              ...
              <div>
                <label className="block text-[11px] text-[#8BBB92] mb-1">Corridor Location</label>
                <input
                  type="text"
                  value={newLocation}
      ...
      {isRevokeModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-neutral-700 bg-[#092328] p-5 shadow-2xl font-mono text-xs">
            <div className="flex items-center justify-between border-b border-[#12544F] pb-3 mb-3">
              ...
              <button onClick={() => setIsRevokeModalOpen(false)} className="text-[#8BBB92] hover:text-[#f0fdf4]">
                <X className="h-4 w-4" />
              </button>
            </div>

            <p className="text-[#8BBB92] mb-3">
