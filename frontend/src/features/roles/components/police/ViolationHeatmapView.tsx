  ...
  const columns: Column<ViolationCorridor>[] = [
    {
      key: 'id',
      header: 'Corridor ID',
      render: (c) => <span className="font-mono text-xs text-[var(--text-secondary)] font-semibold">{c.id}</span>,
    },
    ...
    },
    {
      ...
      align: 'right',
      render: (c) => <span className="font-mono font-bold text-[var(--text-primary)]">{c.violationsCount}</span>,
    },
    {
      key: 'radarActive',
    ...
    },
    {
      ...
      render: (c) => <span className="text-xs text-[var(--text-secondary)]">{c.recommendedAction}</span>,
    },
  ];
    ...
    <div className="flex h-full w-full flex-col gap-3 sm:gap-4 overflow-y-auto p-3 sm:p-5 bg-[var(--surface-canvas)] text-[var(--text-primary)]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-3.5 shrink-0">
        ...
        <MetricCard
          ...
          changeType="negative"
          icon={<Car className="h-4 w-4 text-[var(--text-secondary)]" />}
        />
        <MetricCard
          ...
          changeType="positive"
          icon={<MapPin className="h-4 w-4 text-[var(--text-secondary)]" />}
        />
      </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[var(--text-secondary)]">· {activeCorridor.locationName}</span>
                ...
                <p className="text-xs text-[var(--text-secondary)]">
                  Primary Offense: <span className="text-[var(--text-primary)] font-medium">{activeCorridor.primaryOffense}</span> · Avg Offense Speed: <span className="text-rose-400 font-mono font-bold">{activeCorridor.avgOffenseSpeed}</span>
                </p>
              ...
              <Button
                ...
                className={`text-xs font-bold ${
                  activeCorridor.radarActive
                    ? 'bg-[var(--surface-subtle)] text-[var(--text-secondary)] border-[var(--color-accent-primary)]'
      ...
      {isCameraPreviewOpen && activeCorridor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-xl border border-[var(--surface-border)] bg-[var(--surface-canvas)] p-5 shadow-2xl font-mono text-xs">
            ...
            <div className="relative flex h-64 w-full items-center justify-center rounded-lg border border-[var(--surface-border)] bg-black overflow-hidden mb-4">
              <img
              ...
              <div className="absolute inset-0 pointer-events-none p-3 flex flex-col justify-between bg-gradient-to-t from-black/70 via-transparent to-black/40">
                <div className="flex justify-between text-[10px] text-[var(--text-secondary)]">
                  <span className="bg-black/70 px-2 py-0.5 rounded border border-[var(--surface-border)] text-emerald-400 font-bold">
                    EDGE SENSOR STREAM ACTIVE
                  </span>
                  <span className="bg-black/70 px-2 py-0.5 rounded border border-[var(--surface-border)]">
                    {activeCorridor.id}
                  </span>
                </div>
                <div className="flex justify-between text-[10px] text-[var(--text-secondary)] bg-black/75 px-2 py-1 rounded border border-[var(--surface-border)]">
