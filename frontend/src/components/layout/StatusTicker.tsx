...
export function StatusTicker({
  ...
  return (
    <footer className="flex h-7 w-full items-center justify-between border-t border-[var(--surface-border)] bg-[var(--surface-canvas)] px-3 sm:px-4 text-[11px] font-mono text-[var(--text-secondary)] select-none z-10 shrink-0 overflow-x-auto whitespace-nowrap scrollbar-none">
      {/* Left Metrics */}
      <div className="flex items-center gap-3 sm:gap-4 shrink-0">
        <div className="flex items-center gap-1.5">
          <Wifi className="h-3 w-3 text-[var(--text-secondary)]" />
      ...
      <div className="flex items-center gap-3 sm:gap-4 shrink-0 pl-3">
        <div className="flex items-center gap-1.5">
          ...
          <strong className="text-[var(--text-primary)] tabular-nums">{totalPotholes}</strong>
        </div>

        <div className="hidden md:flex items-center gap-1.5">
          <ShieldCheck className="h-3 w-3 text-[var(--text-secondary)]" />
          <span>Self-Audited:</span>
          <strong className="text-[var(--text-primary)] tabular-nums">{autoVerifiedRepairs}</strong>
        </div>

        <div className="flex items-center gap-1.5">
          <Flame className="h-3 w-3 text-rose-600 dark:text-rose-400" />
          <span>Police Alerts:</span>
          <strong className="text-[var(--text-primary)] tabular-nums">{activeIncidents}</strong>
        </div>
      </div>
    </footer>
  );
